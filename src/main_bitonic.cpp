...
void bitonic_sort(ocl::Kernel &bitonic_global,
    ...
    unsigned int block_sz = 2;

    while (block_sz <= n) {
        unsigned int sz = block_sz / 2;

        while (2 * sz > work_group_size) {
            bitonic_global.exec(gpu::WorkSize(work_group_size, global_work_size), as_gpu, block_sz, sz, n);
            sz /= 2;
        }

        bitonic_local.exec(gpu::WorkSize(work_group_size, global_work_size), as_gpu, block_sz, sz, n);
        block_sz *= 2;
    }
}
...
int main(int argc, char **argv) {
    ...
    as_gpu.resizeN(n);
    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            bitonic_sort(bitonic_global, bitonic_local, as_gpu, n, 256);
            t.nextLap();
        }
