...
int main(int argc, char **argv)
{
    ...
    context.activate();
    {
        ...
        gpu::gpu_mem_32f mem_gpu = gpu::gpu_mem_32f::createN(width * height);

        const unsigned int workGroupSize = 128;

        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(workGroupSize, 1, width, height), mem_gpu, width, height,
                        centralX - sizeX / 2.0f, centralY - sizeY / 2.0f, sizeX, sizeY, iterationsLimit);
            t.nextLap();
        }

        mem_gpu.readN(gpu_results.ptr(), width * height);
